...
// SSHKeyList contains a list of SSHKey instances.
// +kubebuilder:object:root=true
...
// SSHKeySpec defines the desired state of a SSHKey instance.
type SSHKeySpec struct {
	runtimev1.ResourceSpec `json:",inline"`
	ForProvider            SSHKeyParameters `json:"forProvider"`
}

// SSHKeyParameters are the configurable fields of a SSHKey pair.
type SSHKeyParameters struct {
	// Format describes the format of the SSH key.
	// +kubebuilder:validation:Enum:=rsa;ed25519
	// +kubebuilder:default:=ed25519
	Format string `json:"format"`
}

// SSHKeyStatus represents the observed state of a SSHKey instance.
type SSHKeyStatus struct {
	runtimev1.ResourceStatus `json:",inline"`
	AtProvider               SSHKeyObservation `json:"atProvider"`
}

// SSHKeyObservation are the observable fields of a SSHKey instance.
type SSHKeyObservation struct {
	// PublicKey is the public part of the created key
