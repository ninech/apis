...
// +kubebuilder:validation:Enum=C1;C2;C4;nine-standard-1;nine-standard-2;nine-standard-4;nine-highmem-2;nine-highmem-4
...
// furthermore instructs cluster API for a RKE2 cluster to resurrect nodes
// which do not adhere to specific condition states.
// +kubebuilder:subresource:status
// +kubebuilder:printcolumn:name="SYNCED",type="string",JSONPath=".status.conditions[?(@.type=='Synced')].status"
// +kubebuilder:printcolumn:name="READY",type="string",JSONPath=".status.conditions[?(@.type=='Ready')].status"
// +kubebuilder:printcolumn:name="AGE",type="date",JSONPath=".metadata.creationTimestamp"
// +kubebuilder:resource:scope=Namespaced
// +kubebuilder:object:root=true
type NodeResurrector struct {
	metav1.TypeMeta   `json:",inline"`
	metav1.ObjectMeta `json:"metadata,omitempty"`
	Spec              NodeResurrectorSpec   `json:"spec"`
	Status            NodeResurrectorStatus `json:"status,omitempty"`
}

// NodeResurrectorList contains a list of node resurrector instances
// +kubebuilder:object:root=true
type NodeResurrectorList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []NodeResurrector `json:"items"`
}

// A NodeResurrectorSpec defines the desired state of a node resurrector instance.
type NodeResurrectorSpec struct {
	runtimev1.ResourceSpec `json:",inline"`
	ForProvider            NodeResurrectorParameters `json:"forProvider"`
}

// NodeResurrectorParameters are the configurable fields of a node resurrector instance.
type NodeResurrectorParameters struct {
	// Cluster is the cluster where the node resurrector should be deployed
	// to.
	Cluster meta.LocalReference `json:"cluster"`
}

// A NodeResurrectorStatus represents the observed state of a node resurrector instance.
type NodeResurrectorStatus struct {
	runtimev1.ResourceStatus `json:",inline"`
	AtProvider               NodeResurrectorObservation `json:"atProvider"`
}

// NodeResurrectorObservation are the observable fields of a node resurrector instance.
type NodeResurrectorObservation struct {
	// Status of all our child resources.
	meta.ChildResourceStatus `json:",inline"`
	meta.ReferenceStatus     `json:",inline"`
}
